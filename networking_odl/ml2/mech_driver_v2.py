...
class OpendaylightJournalThread(object):
    ...
    @classmethod
    def filter_create_port_attributes(cls, port):
        """Filter out port attributes not required for a create."""
        # TODO(asomya): Add security groups back in
    ...
    @classmethod
    def filter_update_port_attributes(cls, port):
    ...
    def sync_pending_row(self):
        ...
        while True:
            ...
            try:
                ...
                LOG.error("Error syncing %(type)s %(operation)s," +
                          " id %(uuid)s Error: %(error)s",
                          {'type': row.object_type, 'uuid': row.object_uuid,
                           'operation': row.operation, 'error': e.message})
